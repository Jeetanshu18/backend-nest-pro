...
if [ ! -d "$FOLDER_NAME" ]; then
  ...
  git clone https://gitlab.rapidinnovation.tech/root/backend-nest-pro.git -b main "$FOLDER_NAME"
  cd "$FOLDER_NAME"
else
...
if [ $? -eq 0 ]; then
  ...
  docker run -d --network=nest-pro -p 3000:3000 --name "$CONTAINER_NAME" "$IMAGE_NAME"
